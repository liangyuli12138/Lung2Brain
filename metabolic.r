...
# read data
# all cell is tumor
tmp_data=readRDS("/public/workspace/lily/Lung2Brain/inte7/trajectory/LCBM_LC_tumor_trajectory_MYBL2_CEBPB.RDS")
all_data=as.matrix(tmp_data@assayData$exprs) # make sure your data is correct size which can use as.matrix
cell_type=as.vector(tmp_data$type.TF)  # cell type which means cluster or cell type 
...
write.table(mean_expression_noshuffle,file="/public/workspace/lily/Lung2Brain/inte7/metabolism/KEGGpathway_activity_noshuffle.txt",row.names=T,col.names=T,quote=F,sep="\t")
write.table(mean_expression_shuffle,file="/public/workspace/lily/Lung2Brain/inte7/metabolism/KEGGpathway_activity_shuffle.txt",row.names=T,col.names=T,quote=F,sep="\t")
write.table(pvalues_mat,file="/public/workspace/lily/Lung2Brain/inte7/metabolism/KEGGpathway_activity_shuffle_pvalue.txt",row.names=T,col.names=T,quote=F,sep="\t")
